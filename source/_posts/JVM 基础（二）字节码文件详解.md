...
> 使用 Java 语言编写的 源程序 后缀为 `.java`，结果编译之后会生成 `.class` 后缀的文件，这里就是字节码文件了，但是这个字节码文件是通过二进制的方式存储，需要通过特殊的工具`jclasslib`转换成易读的形式
>
> 使用 Java 语言编写的代码有哪些信息呢？让我们想一想吧
>
> - **魔数**：
>   - 你怎么确定这个文件一定是字节码文件呢？
>     - 简单，我就在这个文件的前四个字节标识这个文件是字节码文件。
>   - 怎么表示呢？
>     - 简单，计算机不是二进制的吗，就使用 0 1 来标识吧，那就用`0xcafebabe` 来标识吧。那么这个数干脆就叫**魔数**了。
...
> - **常量池：**在 Java 中定义了标识符（类名、方法名、字段名）和常量，这些数据是在编译之后一般是不变的（即常量）
