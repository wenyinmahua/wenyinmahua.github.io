...
- 工作区：Working Directory ，电脑的本地目录，即在资源管理器上看到的文件夹，实际操作的目录
- 暂存区：Staging Area，也称为索引Index，是一种临时存储区域，由于保存即将提交到Git仓库的修改内容
- 本地仓库：Local Repository，包含了完整的项目历史和元数据

工作区使用 `git add`的指令将修改后的文件提交给暂存区，再通过 `git commit`将暂存区的修改提交给本地仓库。
...
- 未修改（Unmodified）：已被git管理的文件，但是文件内容还没有被修改过

- 已修改（Modified）：已修改的文件还没有增加到暂存区里面
- 已暂存（Staged）：已经提交给暂存区的文件

![image-20250629130338186](https://web-tlias-mmh.oss-cn-beijing.aliyuncs.com/img/image-20250629130338186.png)
...
从上到下逐行匹配，每一行表实一个忽略模式

- 空行或者以 # 开头的行会被 Git 省略。一般空行用于可读性的分割，#一般用作注释
- 使用表中的 Blob 模式匹配，例如：
  - 星号 * 通配任意个字符
  - 问号 ？匹配单个字符
...
- 两个星号 ** 表示匹配任意的中间目录
- 中括号可以使用短中线连接，比如：
  - [0-9]表示任意一个数字，[a-z]表示任意一个小写字幕
- 感叹号！表示取反

```shell
*.txt # 省略所有以txt为尾缀的文件
temp/ # 省略temp文件夹下的所有的文件
```
...
```shell
#设置用户信息，如果没有空格，是不需要加上双引号的 
   git config --global user.name “ma”
   git config --global user.email “ma@ma.cn”
#查看配置信息
   git config --list
...
版本库又叫仓库（Repository），简称Repo，可以理解为一个目录，创建一个仓库之后，会生成一个.git的仓库目录
...
```shell
# 查看当前仓库的状态，查看当前仓库属于哪一个复制，有哪些文件以及这些文件当前处于怎样的状态
git status 
# 查看状态 使输出信息更加简洁
git status –s 
```
...
```shell
# git commit 将暂存区的文件修改提交到本地仓库，不会提交工作区的其他文件，才是真正的被保管起来
git commit -m "日志信息"  <文件名>  
# 没有指定日志信息的时候，会进入一个交互式界面，使用 vim 文本编辑器来编辑提交信息
```
...
```shell
# 取消暂存，注意，对文件修改后，即使没有 commit，执行这个指令不会对文件现有的内容做出改变，即只是将这个文件在暂存区删除
git restore --staged <filename>
git reset HEAD <filename>
# 丢弃所有修改，对于已修改但未提交的文件，通过执行这个指令，放弃修改的内容，退回到上一个版本。
...
- 每次提交都有唯一的提交ID，commit 后面的 16 进制的字符串
- 提交者的昵称、邮箱
- 提交时间
- 提交时编写的注释信息等内容

```shell
# 查看简洁的提交记录，只显示每次提交ID（前七位）和注释信息
git log --oneline
```
...
需要撤销已修改的内容或者回退到之前的某一个版本，可以退回到之前的某一个提交的状态

![image-20250629150415030](https://web-tlias-mmh.oss-cn-beijing.aliyuncs.com/img/image-20250629150415030.png)

git reset 的三种模式（参数），默认是mixed

- soft：回退到某一个版本，并且保留工作区和暂存区的所有修改内容
- hard：回退到某一个版本，并且丢弃工作区和暂存区的所有修改内容
- mixed：回退到某一个版本，并且保留工作区的内容，丢弃暂存区的修改内容

在使用场景方面，soft 和 mixed 的作用基本相似，区别在于是否保存暂存区的内容，一般来说，当连续提交了多个版本，但是觉得这些提交没有太大的意义，可以合并成一个版本的时候，就可以通过这两个参数来进行回退再重新提交。

hard的使用场景是真的要放弃目前本地的修改内容的时候。（需要谨慎使用 hard 这个参数，其实误删也不用担心，git中的所有操作都是可以回溯的，可以使用 git reflog 来查看一下操作的历史记录，然后使用 git reset --hard ID命令来回退到这个版本）

```shell
git reset HEAD^  # 切换到上一个版本
git reset HEAD~  # 切换到上一个版本
git reset HEAD~2  # 切换到前两个版本
```



```shell
mkdir learn-git # 创建learn-git 文件夹
echo 111 > file1.txt # 创建file1.txt 文件，并写入111
echo 222 > file2.txt
echo 333 > file3.txt
git add file1.txt # 将 file1.txt 文件从工作区增加到暂存区
git commit -m "commit1" # 将暂存区的所有的文件增加到本地仓库中
# 重复上面两个操作，将file2.txt 和 file3.txt 的文件保存到本地仓库
# 这里开始进行硬切换版本
git reset --hard 需要退回的版本ID
```
...
```shell
# 比较工作区和暂存区之间的差异内容。显示发生更改的文件和更改的详细信息
git diff # 什么都不加
# 比较工作区和版本库的差异
git diff HEAD
# 比较暂存区和版本库之间的差异
git diff --cached
git diff --staged
# 比较两个特定版本之间的差异，在后面加上想要比较的两个版本的提交ID
git diff c13ea57 143772b
git diff HEAD~2 HEAD # 比较前两个版本和当前版本的差异
git diff HEAD~2 HEAD file.txt # 比较前两个版本和当前版本的file.txt文件的差异
# 比较两个分支之间的差异
git diff <branch_name> <branceh_name>
```
...
git diff
# 运行结果，git diff 的输出
diff --git a/file4.txt b/file4.txt # 提示发生更改的文件
index 435450a..46e9f55 100644 # Git将文件的内容通过哈希算法生成一个40位的哈希值，显示了哈希值的前七位。100644代表文件的权限
--- a/file4.txt
+++ b/file4.txt
@@ -1 +1 @@
-444   # 显示为红色，-代表删除的内容
...
当创建的本地的git仓库的分支名字为master时，如果想修改分支名为 main，可以执行下面的命令

```shell
git branch -M main
```

将本地仓库和远程仓库的 main分支关联起来：

```shell
git push -u origin main:main   #-u 是upstream的缩写，意思是把本地仓库的main分支和远程origin仓库的main分支关联起来
```
...
# 拉取远程仓库内容
git pull <远程仓库名> <远程分支名>:<本地分支名> # 相同可以省略 ：和后面的部分
```

# 12.  命令行-- 分支

> git checkout 命令还可以用来恢复文件或目录到之前的某一个状态
...
# 提交分支至远程仓库
git push <仓库简称> <分支名称>	
# 合并分支 将其他分支合并至当前工作区，合并之后，被合并的分支不会自动shan'chu
git merge <分支名称>
# 删除远程仓库分支
